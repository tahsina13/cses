...
int main() {
    ...
    vector<long long> pre(s.size() + 1, 0); 
    for(int i = 1; i <= s.size(); i++) {
        pre[i] = pre[i - 1] + (s.size() - sa[i]); 
    }
    string ans = ""; 
    int l = 1, r = s.size() + 1; 
    long long ext = 0; 
    for(int i = 0; i < s.size(); i++) {
        int mid = ttl::upper_bound(l, r, [&](int m) {
            return k <= pre[m] - (long long)i * m + ext; 
        }); 
        char ch = s[sa[mid] + i]; 
        l = ttl::upper_bound(l, r, [&](int m) {
            return ch <= s[sa[m] + i]; 
        }); 
        r = ttl::lower_bound(l, r - 1, [&](int m) {
            return ch < s[sa[m] + i]; 
        }) + 1;
        ans += ch; 
        if(k <= pre[l - 1] - (long long)i * (l - 1) + ext + (r - l)) {
            break; 
        }
        ext += r - 1; 
    }
